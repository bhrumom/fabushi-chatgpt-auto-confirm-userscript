...
test('unbound ambiguous send waits 15 minutes before page recovery and keeps the same cadence between refreshes',async()=>{
  const {h,w,dom}=await fixture();
  w.history.pushState({},'', '/c/old-conversation');
  const task={id:'sent-unbound',goal:'review once',mode:'goal',round:1,state:'sending',phase:'review',url:'',token:'one-dispatch',attempted:true,dispatchOriginURL:'https://chatgpt.com/c/old-conversation',sentAt:1_000_000,messages:[]};
  h.data.tasks.push(task);
  h.stopAmbiguousSend(task,false,1_090_001);
  assert.equal(task.state,'sending');
  assert.equal(task.url,'');
  assert.equal(task.token,'one-dispatch');
  assert.equal(task.attempted,true);
  assert.equal(task.ambiguousSendRefreshAttempts||0,0,'90-second confirmation timeout must not immediately reload');
  h.stopAmbiguousSend(task,false,1_899_999);
  assert.equal(task.ambiguousSendRefreshAttempts||0,0,'no page recovery before 15 minutes from the original send');
  h.stopAmbiguousSend(task,false,1_900_000);
  assert.equal(task.ambiguousSendRefreshAttempts,1);
  assert.equal(task.ambiguousSendRefreshAt,1_900_000);
  assert.match(task.messages.at(-1).text,/每 15 分钟/);
  h.stopAmbiguousSend(task,false,2_799_999);
  assert.equal(task.ambiguousSendRefreshAttempts,1,'cooldown prevents a refresh before another 15 minutes');
  task.ambiguousSendRefreshAttempts=4;
  task.ambiguousSendRefreshAt=1_900_000;
  h.stopAmbiguousSend(task,false,2_800_000);
  assert.equal(task.state,'queued');
  assert.equal(task.url,'');
...
test('recovery refresh timing is pinned to 15 minutes for stalled, ambiguous-send, and interruption paths',()=>{
  assert.match(source,/const STALLED_REFRESH_MS = 15 \* 60 \* 1000;/);
  assert.match(source,/const STALLED_REFRESH_COOLDOWN_MS = STALLED_REFRESH_MS;/);
  assert.match(source,/const AMBIGUOUS_SEND_REFRESH_MS = STALLED_REFRESH_MS;/);
  assert.match(source,/const CONNECTION_INTERRUPTED_REFRESH_COOLDOWN_MS = 15 \* 60 \* 1000;/);
  assert.match(source,/stalledFor >= STALLED_REFRESH_MS/);
