# Fabushi 独立油猴工作台 2.9.15

这是 Fabushi 的独立油猴脚本源码仓库：
`https://github.com/bhrumom/fabushi-chatgpt-auto-confirm-userscript`。
入口文件是 `chatgpt-auto-confirm.user.js`，当前版本为 `2.9.15`。
...
轻量验证（2026-09-13）：`node --check chatgpt-auto-confirm.user.js` 通过；`npm test` 共 87 项通过，其中新增根路径加载转圈阻止派发、表单外原生文件控件回退、旧附件超时记录安全恢复回归。在线文件曾因传输截断提示导致 2.9.14 无法启动，2.9.15 已用完整源码覆盖并递增版本号，确保油猴重新拉取；ChatGPT 实站上传仍需在更新后由用户使用实际图片/视频验证。
