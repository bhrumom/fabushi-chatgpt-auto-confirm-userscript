...
- 连接中断：ChatGPT 页面级提示出现“连接已中断。正在等待完整回复。”时，插件保留当前任务、会话链接和发送标识；首次检测不会立即刷新，连续 15 分钟仍未恢复后才执行第 1/3 次刷新，之后第 2/3、3/3 次刷新之间也至少间隔 15 分钟。第 3 次刷新后提示仍存在，就停止继续刷新并在原会话输入发送“继续完成所有”。提示曾消失仍由当前持久化会话状态判断，不读取聊天正文或插件自己的日志作为中断触发。
...
### 2.9.46 恢复刷新统一为 15 分钟

- 普通绑定会话不再在连续 3 分钟无变化时刷新；阈值与后续刷新冷却统一改为 15 分钟。
- 未绑定的发送结果不明确场景也不再在 90 秒确认超时后立即刷新；从原发送时间起满 15 分钟才允许首次页面恢复，之后每次恢复同样至少间隔 15 分钟。
- “连接已中断。正在等待完整回复。”不再使用 10 秒快速刷新：首次检测先等待 15 分钟，第 1/3、2/3、3/3 次刷新之间均至少间隔 15 分钟；3/3 后仍中断继续沿用原会话持久 pending continuation，发送“继续完成所有”。
- 本节覆盖 2.9.32、2.9.33、2.9.39 与 2.9.43 中关于 180 秒/3 分钟/10 秒刷新节奏的历史描述；其它完成识别、授权、限流和同会话续发边界不变。
