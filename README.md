...
- 连接中断：ChatGPT 页面级提示出现“连接已中断。正在等待完整回复。”时，插件保留当前任务、会话链接和发送标识，以 3 分钟冷却最多刷新同一 URL 3 次；第 3 次刷新后提示仍存在，就停止继续刷新并在原会话输入发送“继续完成所有”。提示曾消失则这组三次计数清零，下一次中断重新从第 1 次开始。聊天正文或插件自己的日志包含相同文字不会触发。
