# Fabushi 独立油猴工作台 2.9.73

这是 Fabushi 的独立油猴脚本源码仓库：
`https://github.com/bhrumom/fabushi-chatgpt-auto-confirm-userscript`。
入口文件是 `chatgpt-auto-confirm.user.js`，当前版本为 `2.9.73`。
...
## 验证记录（2026-09-25，2.9.73）

运行 `npm test`：208 项测试中 201 项通过、7 项明确跳过、0 项失败。授权回归覆盖新版“允许一次 + aria-hidden 快捷键 + 审批选项”真实结构、真实 Radix `pointerdown` 菜单、正文无授权关键词的结构化卡片识别、普通“允许/允许一次”按钮排除、GitHub 连接器命名的本次会话授权、永久授权拒绝、非脚本会话授权，以及 ChatGPT 弹窗自动关闭且不误关新版授权卡；其余调度、附件、恢复、会话身份、限流、最终回复和内存回归也全部保持通过。
