# Fabushi 独立油猴工作台 2.9.47

这是 Fabushi 的独立油猴脚本源码仓库：
`https://github.com/bhrumom/fabushi-chatgpt-auto-confirm-userscript`。
入口文件是 `chatgpt-auto-confirm.user.js`，当前版本为 `2.9.47`。
...
- 连接中断：ChatGPT 页面级提示出现“连接已中断。正在等待完整回复。”时，插件保留当前任务、会话链接和发送标识。首次检测不会立即刷新，连续 15 分钟仍未恢复后才执行第 1/3 次刷新；之后第 2/3、3/3 次刷新之间也至少间隔 15 分钟。第 3 次刷新后提示仍存在，就停止继续刷新并在原会话输入发送“继续完成所有”。同一会话的中断计数可跨短暂 loading/generating hydration 保持，并在 conversation URL 改变、真正续发或真正最终回复时清零；聊天正文或插件自己的日志包含相同文字不会触发。
