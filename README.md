# Fabushi 独立油猴工作台 2.9.36

这是 Fabushi 的独立油猴脚本源码仓库：
`https://github.com/bhrumom/fabushi-chatgpt-auto-confirm-userscript`。
入口文件是 `chatgpt-auto-confirm.user.js`，当前版本为 `2.9.36`。
...
最后一条用户消息必须包含本轮随机标识。当前 assistant 回复还必须有本轮完成控件或明确结束标记、没有停止按钮、没有待授权卡，并稳定至少 4 秒；其中可见的“复制”按钮加“分享”、评价回复或点赞/点踩反馈按钮就是完成控件，不再要求额外的重新生成/更多/分支按钮。普通文字暂时不变化不算完成。若停止按钮消失后连续 15 秒仍没有最终证据或授权卡，会在新 Chat 原样恢复；重载后无法确认曾经生成时仍使用 5 分钟兜底，最多自动重试 4 次。Work 完成后会把自然语言结果写入同一任务的 `result`，再由新的规划/验收会话依据原目标、附件和该结果判断是否进入下一轮。
