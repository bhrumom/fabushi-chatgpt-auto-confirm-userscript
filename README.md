...
- 连接中断：ChatGPT 页面级提示出现“连接已中断。正在等待完整回复。”时，插件保留当前任务、会话链接和发送标识，并使用独立的 10 秒恢复节流快速完成 1/3、2/3、3/3 次同会话刷新；第 3 次刷新后提示仍存在，就停止继续刷新并在原会话输入发送“继续完成所有”。同一会话的中断计数可跨短暂 loading/generating hydration 保持，并在 conversation URL 改变、真正续发或真正最终回复时清零；聊天正文或插件自己的日志包含相同文字不会触发。
...
### 2.9.48 15 分钟仅用于通用停滞刷新

- 更正 2.9.47 的范围误解：用户要求的“15 分钟内页面没有变化才刷新”只适用于通用 generic stall refresh，不适用于明确的“连接已中断”错误恢复。
- generic stall 继续保持 15 分钟；未绑定 ambiguous-send 继续保持 3 分钟；连接中断恢复恢复为独立 10 秒节流，仍是 1/3、2/3、3/3 后原会话 durable `继续完成所有`。
- 2.9.47 未作为正式 Release 发布；2.9.48 用更高版本号确保曾读取过 2.9.47 main 元数据的客户端能够更新到修正后的行为。
