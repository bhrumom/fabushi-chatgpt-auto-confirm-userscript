# Fabushi 独立油猴工作台 2.9.40

这是 Fabushi 的独立油猴脚本源码仓库：
`https://github.com/bhrumom/fabushi-chatgpt-auto-confirm-userscript`。
入口文件是 `chatgpt-auto-confirm.user.js`，当前版本为 `2.9.40`。
...
- 连接中断：ChatGPT 页面级提示出现“连接已中断。正在等待完整回复。”时，插件保留当前任务、会话链接和发送标识，只刷新同一 URL；如果该中断状态持续超过 30 分钟，就在原会话输入并发送“继续完成所有”。之后仍未得到最终回复时继续采用同会话恢复，只有真正最终回复确认后才进入下一轮新会话。聊天正文或插件自己的日志包含相同文字不会触发。
