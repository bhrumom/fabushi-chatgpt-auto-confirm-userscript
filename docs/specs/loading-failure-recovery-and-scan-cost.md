...
| R17 | passed locally | A page with 500 unrelated labels and a 20,000-repeat transcript confirms request-frequency checks do not read the full `<main>` text. Existing history-only and real rate-limit tests pass. Full `npm test`: 224 total, 217 passed, 0 failed, 7 skipped; syntax and diff checks pass. Live Chrome acceptance and release CI pending. |
